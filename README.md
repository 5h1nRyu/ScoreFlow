...
每条折线右侧会显示对应队伍的 `name`。标签随当前分数移动，当分数排名交换时会平滑交换位置；标签过近时会自动产生竖向偏移，并用引导线指向所属折线。

可在 `config.js` 的 `lineLabels` 中调整：

- `enabled`：标签显示开关；
- `fontSize`：标签字号（CSS 像素）；
- `acceleration`：标签追赶目标位置的速率，数值越大跟随越快；
- `horizontalGap`：标签与折线末端的水平间距；
- `minimumGap`：标签之间额外保留的最小竖直间距。
